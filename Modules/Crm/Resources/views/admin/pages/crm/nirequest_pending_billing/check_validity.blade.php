            ...
            <div class="main-body">
                <div class="row gutters-sm">
                    ...
                    <div class="col-md-12 mb-1">
                        <form action="{{ route('optimize_pending_billing.confirm', $optimize->id) }}"
                            method="post">
                            @csrf
                            ...
                            <table class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="padding: 10 5 !important;" width="12%"> COMPANY</th>
                                ...
                                <tbody>
                                    @if(isset(App\Models\BandwidthSaleInvoice::where('billing_month', date('F',strtotime($optimize->apply_date)) ." ". date( 'Y',strtotime($optimize->apply_date)))->where('customer_id',$optimize->customer_id)->first()->detaile))
                                    @php
                                        $packages = App\Models\BandwidthSaleInvoice::where('billing_month', date('F',strtotime($optimize->apply_date)) ." ". date( 'Y',strtotime($optimize->apply_date)))->where('customer_id',$optimize->customer_id)->first()->detaile->groupBy('item_id');
                                    @endphp

                                    @forelse ($packages as $iteml)
                                       @forelse ($iteml as $value)
                                         ...
                                         <tr>
                                            <td>
                                             ...
                                             <th>
                                                 <select name="item_id[]" class="form-control d-none item_id">
                                                     ...
                                                     @foreach ($items as $item)
                                                         <option {{ $value->item_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }} </option>
                                                     @endforeach
                                                 </select>
                                                 {{ $value->getItem->name ?? '' }}
                                             </th>
                                             ...
                                             <th>
                                                 {{ $value->qty ?? '' }}
                                                 <input type="text" value="{{ $value->qty ?? '' }}" name="qty[]" class="form-control d-none qty calculation">
                                             </th>

                                             <th>
                                                 {{ $value->rate ?? '' }}
                                                 <input type="text" value="{{ $value->rate ?? '' }}" name="rate[]" class="form-control d-none rate calculation">
                                             </th>

                                             <th>
                                                 {{ $value->vat ?? '' }}
                                                 <input type="text" value="{{ $value->vat ?? '' }}" name="vat[]" class="form-control d-none vat calculation">
                                             </th>

                                             <th>
                                                 <input type="date" value="{{ $value->from_date }}" name="from_date[]" class="form-control from_date calculation">
                                             </th>

                                              @if($loop->last && (count($iteml) > 0) && in_array($value->item_id,$package->item_id))
                                              <th>
                                                  <input type="date" value="{{ date('Y-m-d', strtotime($optimize->apply_date . '-1 day'))}}"
                                 ...
                                 @else
                                    ...
                                    @forelse ($customerDetail->package as $value)
                                    @if(!in_array($value->item_id,$package->item_id))
                                         <tr>
                                            <td>
                                                <select name="business_id[]" class="form-control">
                                                    @foreach ($businesses as $item)
                                             ...
                                             <th>
                                                 <input type="date" value="{{ $optimize->apply_date }}" name="from_date[]" class="form-control from_date calculation">
                                             </th>
                                              <th>
                                                  <input type="date" value="{{ date('Y-m-d', strtotime("last day of this month"))}}"
                                                      name="to_date[]" class="form-control to_date calculation">
                                              </th>
                                             <th>
                                                 <input type="text" value="0" readonly name="total[]"
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @foreach ($package->item_id as $key => $value)
                                        ...
                                        <tr>
                                            ...
                                            <th>
                                                {{ $package->total_quantity[$key]  }}
                                                <input type="text"
                                                    value="{{ $package->total_quantity[$key]}}"
                                            ...
                                            <th>
                                                <input type="date" value="{{ $optimize->apply_date }}" name="from_datenew[]"
                                ...
                                <tfoot>
                                    ...
                                    <tr style="background-color: rgb(194, 194, 194);">
                                        <td colspan="7"><span>Sub Total</span></td>
                                        <td><input type="text" value="0" id="SubTotal" class="form-control"
                                                disabled=""></td>
                                    </tr>
                                </tfoot>
                            </table>
...
@section('scripts')
    <script>
        ...
        function subtotal() {
            ...
            let totalamount = grandtotal1 + grandtotal;
