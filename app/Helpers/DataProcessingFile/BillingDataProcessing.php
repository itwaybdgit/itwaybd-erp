            ...
            foreach ($results as $key => $item) {
                foreach ($tableColumnsName as $columnItem) {
                    if (array_key_exists('data', $columnItem)) {
                        ...
                        if ($columnItem['data'] == 'totalamount') {
                            $billings = Billing::where('customer_id', $item->customer_id)->where('status', '!=', 'paid')->get();
                            $total = 0;
                            foreach ($billings as $billing) {
                                $total +=  intval($billing->customer_billing_amount) - $billing->pay_amount;
                            }
