...
trait BtrcReportDataProcessing
{
            ...
            foreach ($results as $key => $item) {
                foreach ($tableColumnsName as $columnItem) {
                    if (array_key_exists('data', $columnItem)) {
                        ...
                        if ($columnItem['data'] == 'connection_type') {
                            $nestedData['connection_type'] = "Wired";
                        }

                        if ($columnItem['data'] == 'thana') {
                            $nestedData['thana'] = "Kamtoli";
                        }

                        if ($columnItem['data'] == 'DC') {
                            $nestedData['DC'] = "DC";
                        }

                        if ($columnItem['data'] == 'connectivity_type') {
                            $nestedData['connectivity_type'] = "Shared";
                        }

                        if ($columnItem['data'] == 'connection_date') {
                            $nestedData['connection_date'] = Carbon::parse(($nestedData['connection_date'] == "N/A" ? date('Y-m-d') :$nestedData['connection_date'] ))->format('d/m/Y');
                        }

                        if ($columnItem['data'] == 'division') {
                            $nestedData['division'] = "Dhaka";
                        }

                        if ($columnItem['data'] == 'district') {
                            $nestedData['district'] = "Dhaka";
                        }
