...
trait CustomerSupportTicketDataProcessing
{
                    ...
                    if (array_key_exists('data', $columnItem)) {
                        ...
                        if (isset($columnItem['relation']) && !empty($columnItem['relation']) && !isset($columnItem['fun'])) {
                            ...
                            if ($columnItem['relation'] == "supportstatus") {
                                if ($item->status == 1) {
                                    $button = "<a href='" . route('bandwidthcustomer.supportticket.status', $item->id) . "' class='btn btn-square btn-danger'>".$item->supportstatus->name ?? ""."</a>";
                                } elseif ($item->status == 2) {
                                    $button = "<a href='" . route('bandwidthcustomer.supportticket.status', $item->id) . "' class='btn btn-square btn-warning'>".$item->supportstatus->name ?? ""."</a>";
                                } else {
                        ...
                        if ($columnItem['data'] == 'complain_number') {
                            $nestedData['complain_number'] = "<a href='".route('bandwidthcustomer.supportticket.status', $item->id)."'>".$item->complain_number."</a>";
                        }
