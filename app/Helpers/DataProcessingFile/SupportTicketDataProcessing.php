...
trait SupportTicketDataProcessing
{
                ...
                foreach ($tableColumnsName as $columnItem) {
                    if (array_key_exists('data', $columnItem)) {
                        ...
                        if (isset($columnItem['relation']) && !empty($columnItem['relation']) && !isset($columnItem['fun'])) {
                            ...
                            if ($columnItem['relation'] == "supportstatus") {
                                if ($item->status == 1) {
                                    $button = "<a href='" . route('supportticket.status', $item->id) . "' class='btn btn-square btn-danger'>".$item->supportstatus->name ?? ""."</a>";
                                } elseif ($item->status == 2) {
                                    $button = "<a href='" . route('supportticket.status', $item->id) . "' class='btn btn-square btn-warning'>".$item->supportstatus->name ?? ""."</a>";
                                } else {
                                    $button = "<button  class='btn btn-square btn-success'>".$item->supportstatus->name ?? ""."</button>";
                                }
                                $nestedData[$columnItem['relation'] . '_' . $columnItem['data']] =  $button ;
                            }
                        }
                        ...
                        if ($columnItem['data'] == 'complain_number') {
                            $nestedData['complain_number'] = "<a href='".route('supportticket.status', $item->id)."'>".$item->complain_number."</a>";
                        }
                        if ($columnItem['data'] == 'attachments') {
                            if($item->attachments){
                                $nestedData['attachments'] = "<a download href='".asset('storage/'.$item->attachments)."'><i class='fas fa-download'></i></a>";
                            }else{
                                $nestedData['attachments'] = "No File";
                            }
                        }

                        if ($columnItem['data'] == 'complain_time') {
                            $nestedData['complain_time'] =  date("Y-m-d (g:i a)", strtotime($item->complain_time));
                        }
