                ...
                foreach ($tableColumnsName as $columnItem) {
                    if (array_key_exists('data', $columnItem)) {
                        ...
                        if ($columnItem['data'] == 'amount') {
                                $nestedData['amount'] =($item->package->sum('qty') ?? 0) * ($item->package->sum('rate') ?? 0) ;
                        }

                        if (isset($columnItem['qty'])) {
                            $qty = $item->package->where('item_id',$columnItem['data'])->first();
                            $nestedData[$columnItem['data']] = $qty->qty ?? 0;
                        }

                        if (isset($columnItem['value'])) {
                            $qty = $item->package->where('item_id',$columnItem['data'])->first();
                            $nestedData[$columnItem['data']] = (($qty->rate) ?? 0);
                        }

                        if ($columnItem['data'] == 'created_at') {
                            $nestedData['created_at'] = date("d-m-Y",strtotime($item->created_at)) ;
                        }

                        if ($columnItem['data'] == 'bandwidth') {
                            $nestedData['bandwidth'] = $item->package->sum('qty') ?? 0 ;
                        }
