...
class ApiController extends Controller
{
    ...
    public function get_subzones(Request $request)
    {
        ...
        $options = '<option  selected value="0">Select Option</option>';
    ...
    public function get_splitter(Request $request)
    {
        $this->validate($request, [
            'model_name' => ['required'],
            'model_id' => ['required'],
        ...
        if ($request->model_id) {
            $splitters = $request->model_name::where($request->column_name, $request->model_id)->get();
        }
        ...
        if ($splitters->isNotEmpty()) {
            foreach ($splitters as $splitter) {
                $options .= '<option value="' . $splitter->id . '">' . $splitter->name . ' </option>';
            }
        }
