                    ...
                    <div class="card-datatable table-responsive">
                        <table id="server_side_lode" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                ...
                                @php
                                    $bill = \App\Models\AccountTransaction::where('company_id', auth()->user()->company_id)->where('account_id',"!=",'5')->where('type', 4)->whereDate('created_at', date('Y-m-d'))->where('table_id',$billing->id)->sum('debit') ?? 0;
                                @endphp
                                <tr>
                                    <td>{{ $key + 1 }}</td>
