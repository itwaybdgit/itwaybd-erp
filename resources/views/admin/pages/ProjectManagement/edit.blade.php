            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                        <h4 class="card-title">{{ $page_heading ?? 'Project Edit' }}</h4>
                            ...
                            <div class="row g-3 mt-2">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="date" name="starting_date" id="starting_date" class="form-control"
                                        value="{{ old('starting_date', $project->starting_date) }}" required>
                                    @error('starting_date')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="date" name="ending_date" id="ending_date" class="form-control"
                                        value="{{ old('ending_date', $project->ending_date) }}" required>
                                    @error('ending_date')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                    ...
                                    <select name="team_members[]" id="team_members" class="form-select select2-multiple" multiple>
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" 
                                                ...
                                                {{ in_array($user->id, $selectedMemberIds ?? []) ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row g-3 mt-2">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select name="hypercare" id="hypercare" class="form-select form-control" required>
                                        <option value="0" {{ old('hypercare', $project->hypercare) == '0' ? 'selected' : '' }}>No</option>
                                        <option value="1" {{ old('hypercare', $project->hypercare) == '1' ? 'selected' : '' }}>Yes</option>
                                    </select>
                                    @error('hypercare')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                ...
                                <div class="col-md-4" id="hypercare_months_div" style="display: none;">
                                    ...
                                    <input type="number" name="hypercare_months" id="hypercare_months" class="form-control"
                                        min="1" value="{{ old('hypercare_months', $project->hypercare_months) }}">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="number" name="estimated_hours" id="estimated_hours" class="form-control"
                                        min="0" value="{{ old('estimated_hours', $project->estimated_hours) }}">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="budget" id="budget" class="form-control" 
                                            step="0.01" min="0" value="{{ old('budget', $project->budget) }}">
                                    </div>
                                    @error('budget')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                        ...
                                        @php
                                            ...
                                            if (!$selectedTags) {
                                                if (is_string($project->tags)) {
                                                    ...
                                                    $selectedTags = is_array($decoded) ? $decoded : array_filter(explode(',', $project->tags));
                                                } elseif (is_array($project->tags)) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('#tags').select2({
                ...
                tags: true,
                tokenSeparators: [',', ' '],
                maximumSelectionLength: 10, // Limit number of tags
                ...
                templateResult: function(state) {
                    if (state.newTag) {
                        return $('<span class="text-primary"><i class="fa fa-plus"></i> Add "' + state.text + '"</span>');
                    }
                    return state.text;
                },
