                    ...
                    <div class="container task-form-container">
                        ...
                        <form action="{{ route('task.update', $task->id) }}" method="POST" id="taskForm"
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="datetime-local" class="form-control" id="startDateTime"
                                        name="start_date_time" value="{{ old('start_date_time', date('Y-m-d\TH:i', strtotime($task->start_date_time))) }}" required />
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="datetime-local" class="form-control" id="endDateTime" name="end_date_time"
                                        value="{{ old('end_date_time', date('Y-m-d\TH:i', strtotime($task->end_date_time))) }}" required />
                                    @error('end_date_time')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                    ...
                                    @if($task->image)
                                        <div class="mt-2">
                                            ...
                                            <div class="current-image-preview">
                                                <img src="{{ Storage::url($task->image) }}" alt="Current task image"
                            ...
                            <div class="mb-4">
                                ...
                                <div class="alert alert-info" id="teamWarning" style="display: none;">
                                ...
                                <div id="subtasksContainer" class="subtasks-container">
                                    <!-- Existing subtasks will be populated here -->
                                    @if($task->subtasks && $task->subtasks->count() > 0)
                                        @foreach($task->subtasks as $index => $subtask)
                                            <div class="subtask-item" data-subtask-id="{{ $index + 1 }}">
                                                ...
                                                <!-- Hidden field for existing subtask ID -->
                                                ...
                                                <div class="row subtask-row">
                                                    ...
                                                    <div class="col-md-4">
                                                        ...
                                                        <select class="form-select form-control" id="subtask_user_{{ $index + 1 }}"
                                                                name="subtasks[{{ $index + 1 }}][user_id]" required>
                                                            <option value="" disabled>Select employee</option>
                                                            @foreach($teamEmployees as $employee)
                                                                <option value="{{ $employee->id }}"
                                                                    {{ old('subtasks.'.($index + 1).'.user_id', $subtask->user_id) == $employee->id ? 'selected' : '' }}>
                                                                    {{ $employee->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                ...
                                                <div class="row subtask-row">
                                                    <div class="col-md-6">
                                                        <label for="subtask_priority_{{ $index + 1 }}" class="form-label">Priority</label>
                                                        <select class="form-select form-control" id="subtask_priority_{{ $index + 1 }}"
                                                                name="subtasks[{{ $index + 1 }}][priority]">
                                                            <option value="" disabled>Select priority</option>
                                                    ...
                                                    <div class="col-md-6">
                                                        <label for="subtask_status_{{ $index + 1 }}" class="form-label">Status</label>
                                                        <select class="form-select form-control" id="subtask_status_{{ $index + 1 }}"
                                                                name="subtasks[{{ $index + 1 }}][status]">
...
@section('scripts')
    <script>
        let subtaskCounter = {{ $task->subtasks->count() }}; // Start with existing subtasks count
        let teamEmployees = @json($teamEmployees); // Pre-load team employees
        let selectedTeam = @json($task->team); // Pre-load selected team
        let mainTaskTitle = '{{ $task->title }}';
        let teamProjects = @json($teamProjects); // Pre-load team projects
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // If we have a team selected, enable add subtask button
            if (selectedTeam) {
        ...
        document.getElementById('teamSelect').addEventListener('change', function() {
            ...
            if (teamId) {
                ...
                // Find selected team from the teams data
        ...
        document.getElementById('imageUpload').addEventListener('change', function(e) {
            ...
            if (file && file.type.startsWith('image/')) {
                ...
                reader.onload = function(e) {
                    ...
                    img.alt = 'New uploaded image';
                    img.style.cssText = 'max-width: 100px; max-height: 100px; object-fit: cover; border-radius: 5px;';
                    fileItem.appendChild(img);
                    fileItem.appendChild(document.createTextNode(' ' + file.name));
                    imagePreview.innerHTML = '<small class="text-muted d-block">New image:</small>';
