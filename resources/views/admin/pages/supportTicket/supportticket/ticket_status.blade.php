                                ...
                                @foreach (json_decode($supportticket->attachments, true) as $filePath)
                                    <div class="col-3">
                                        <div class="file-preview text-center">
                                            ...
                                            <div class="mt-2">
                                                <input type="checkbox" name="remove_files[]" value="{{ $filePath }}">
                                                Remove
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <form action="{{ route('my_supportticket.statusup', $supportticket->id) }}" method="post">
                                    ...
                                    <table class="table table-borderless">
                                        ...
                                        <tr>
                                            <td>Assigned By</td>
                                            <td>
                                                <select {{ in_array($supportticket->status, [1, 2]) ? '' : 'disabled' }}
                                                    name="assign_to" id="" class="form-control select2">
                                                    <option value="0" selected value="">
                                                        {{ $supportticket->assignUser->name ?? '' }}</option>
                                                    @foreach ($employees as $employee)
                                                        <option
                                                            {{ $supportticket->assign_to == $employee->user_id ? 'selected' : '' }}
                                                            value="{{ $employee->user_id }}">{{ $employee->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </td>
                                        ...
                                        <tr>
                                            ...
                                            <td> <select name="status" id="" class="form-control">
                                                    @foreach ($status as $statu)
                                                        <option
                                                            {{ $supportticket->status == $statu->id ? 'selected' : '' }}
                                        ...
                                        @if (in_array($supportticket->status, [1, 2]))
                                            <tr>
                                                <td></td>
                                                <td>
                                                    <button class="btn btn-info">Submit</a>
                                                </td>
                                            </tr>
                                        @endif
                                    </table>
