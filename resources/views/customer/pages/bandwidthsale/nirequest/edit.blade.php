        ...
        <div class="col-xl-12 col-lg-12">
            <div class="card">
                        ...
                        <div class="d-flex">
                            <select name="customer" class="form-control select2 w-100" id="customer_down">
                                ...
                                @foreach ($customers as $customer)
                                    <option {{ $optimize->customer_id == $customer->id ? 'selected' : '' }}
                ...
                <div class="card-body">
                    ...
                    <x-alert></x-alert>
                    @if ($selectedCustomer)
                        <form action="{{ route('nirequest.store') }}" method="post">
                            @csrf
                            <input type="hidden" name="customer" value="{{ $optimize->customer_id }}">
                            <table id="table3" class="   table-responsive">
                                    ...
                                    @foreach ($selectedCustomer as $selected)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                $change = "";
                                                $curent = "";
                                                 $change_quantity = json_decode($optimize->package);
                                                 foreach($change_quantity->item_id as $key => $itemid){
                                                    if($selected->item_id == $itemid){
                                                        $change =  $change_quantity->change_quantity[$key];
                                                        $curent =  $change_quantity->quantity[$key];
                                                    }
                                                 }
                                                @endphp
                                                <input type="number" value="{{$change}}" name="change_quantity[]"
                                                    class="form-control qty calculation"
                                                    >
                                            </td>
                                            <td>

                                                <input type="number" value="{{$curent}}" name="quantity[]"
                                                    class="form-control minusqty calculation"
                                                    >
                                            </td>
                                            <td>
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="6"></td> <!-- Colspan 5 for empty cells under "Price" -->
                                        <td colspan="1">
                                            <input type="text" value="" name="total2" class="form-control mt-1"
                                                readonly>
                                        </td>
                                        <td colspan="1">
                                            <input type="text" value="" name="total1" class="form-control mt-1"
                                                readonly>
                                        </td>
        ...
        $(document).ready(function() {
            ...
            $.each($('.qty'), function() {
                if($(this).val() != ""){
                    ...
                    var oldQuantity1 = parseFloat(row1.find('.old-quantity').val());
                    let val1 = ($(this).val() == "") ? 0 : $(this).val();
                    var newQuantity1 = parseFloat(val1);
                    var totalQuantity1 = oldQuantity1 + newQuantity1; // Calculate total quantity
                    console.log(totalQuantity1);
                    row1.find('.total-quantity').val(totalQuantity1); // Set total quantityd
                    calculateTotals(); // Recalculate totals
                }
            });

            $.each($('.minusqty'), function() {
                if($(this).val() != ""){
                var row = $(this).closest('tr');
                var oldQuantity = parseFloat(row.find('.old-quantity').val());
                let val = ($(this).val() == "") ? 0 : $(this).val();
